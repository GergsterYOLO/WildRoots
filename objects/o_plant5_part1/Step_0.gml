...
if (global.nbvisible) {
    ...
    if (id == o_plant5_part1) {
        global.plant5_part1_pos = [center_x - 410, center_y];
    }
	
	//if already found
	var mountain_map = global.notebook[? "Mountain"];
	if (mountain_map[? "aspects_discovered"][0]) {
		visible = true;
    }
    ...
    if (mouse_check_button_pressed(mb_left) && position_meeting(mouse_x, mouse_y, id)) {
        ...
        //update map
        if (object_index == o_plant5_part1) {
			mountain_map[? "aspects_discovered"][0] = true;
        }
        //update plant entry visibility
        var entry_instance = instance_find(o_plant5_part1_entry, 0);
