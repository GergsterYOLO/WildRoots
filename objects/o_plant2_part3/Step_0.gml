...
if (global.nbvisible) {
    ...
    if (id == o_plant2_part3) {
        global.plant2_part3_pos = [center_x - 410, center_y];
    }
	
	//if already found
	var globemallow_map = global.notebook[? "Globemallow"];
	if (globemallow_map[? "aspects_discovered"][2]) { //1
		visible = true;
    }
    ...
    if (mouse_check_button_pressed(mb_left) && position_meeting(mouse_x, mouse_y, id)) {
        ...
        //update map
        if (object_index == o_plant2_part3) {
			globemallow_map[? "aspects_discovered"][2] = true;
        }
        //update plant entry visibility
        var entry_instance = instance_find(o_plant2_part3_entry, 0);
