...
if (global.nbvisible) {
    ...
    if (id == o_plant5_part2) {
        global.plant5_part2_pos = [center_x - 410, center_y];
    }
	
	//if already found
	var mountain_map = global.notebook[? "Mountain"];
	if (mountain_map[? "aspects_discovered"][1]) {
		visible = true;
    }
    ...
    if (mouse_check_button_pressed(mb_left) && position_meeting(mouse_x, mouse_y, id)) {
        ...
        //update map
        if (object_index == o_plant5_part2) {
			mountain_map[? "aspects_discovered"][1] = true;
        }
        //update plant entry visibility
        var entry_instance = instance_find(o_plant5_part2_entry, 1);
